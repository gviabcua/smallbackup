<?php namespace Gviabcua\SmallBackup\Controllers;
...
use Gviabcua\SmallBackup\Classes\{DbBackupManager, PluginsBackupManager, ThemeBackupManager, StorageBackupManager};
...
class Settings extends SystemSettings
{
    ...
    /**
     * List of plugins backups
     *
     * @return array
     */
    public function getPluginsBackupList(): array
    {
        return (new PluginsBackupManager)->list();
    }
    /**
    ...
    public function update_onBackup()
    {
        try {
            ...
            $manager = new PluginsBackupManager;
            $deleted += $manager->clear();
            $files[] = $manager->backup();
            ...
            Flash::success(trans('gviabcua.smallbackup::lang.backup.flash.backup_all', ['deleted' => $deleted, 'files' => implode(', ', $files)]));
    ...
    public function update_onBackupDb()
    {
        try {
            ...
            Flash::success(trans('gviabcua.smallbackup::lang.backup.flash.backup_all', ['deleted' => $deleted, 'files' => implode(', ', $files)]));
            return \Backend::redirect('gviabcua/smallbackup/settings/update');
        }
        catch (Exception $ex) {
            Log::error($ex->getMessage());
            Flash::error($ex->getMessage());
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update_onBackupPlugins()
    {
        try {
            $manager = new PluginsBackupManager;
            $deleted = $manager->clear();
