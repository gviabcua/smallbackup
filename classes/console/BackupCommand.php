<?php namespace Gviabcua\SmallBackup\Classes\Console;

use Exception;
use Illuminate\Console\Command;
use Symfony\Component\Console\Input\InputOption;
use Symfony\Component\Console\Input\InputArgument;
use Gviabcua\SmallBackup\Classes\BackupManager;
    ...
    protected function cleanup(BackupManager $manager)
    {
        if (!$this->option('no-cleanup')) {
            try {
                ...
                $this->output->success(
                    trans('gviabcua.smallbackup::lang.backup.flash.expired_deleted', ['deleted' => $deleted])
                );
            } catch (Exception $ex) {
